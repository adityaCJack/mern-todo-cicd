...
const TodosList = () => {
  ...
  const { isLoading, todos, todos2, error, isLoadingPost, isLoadingDelete } =
    useSelector((state) => state.todos);

  useEffect(() => {
    getAllTodos(dispatch);
    getAllTodos2(dispatch);
  }, [isLoadingPost, isLoadingDelete]);
  ...
  return (
    <div className="flex flex-col items-center justify-center w-[90%] md:w-[80%] lg:w-[50%] my-5">
      ...
      {todos2?.length === 0 ? (
        <h2 className="text-black text-2xl font-bold my-2">
          Sorry! No Todos Found. From 2nd microservice
        </h2>
      ) : (
        todos2?.todos?.map((todo, index) => (
          <div
            key={index}
            className="bg-gray-100 w-full rounded-sm shadow-md p-5 my-2 transition-all duration-100 hover:shadow-lg"
          >
